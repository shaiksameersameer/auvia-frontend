...
export const WhatIsAutism = () => {
  ...
  const sections = [
    ...
    },
    {
      ...
      icon: <HeartHandshake className="text-brand-peach" size={48} />
    },
    {
      id: "goals",
      title: "Therapy",
    ...
    <div className="max-w-2xl">
      ...
      <motion.div initial="hidden" animate="visible" variants={stagger}>
        ...
        <motion.div variants={fadeInUp} className="backdrop-blur-[2px] bg-white/10 p-4 rounded-[32px] -ml-4">
          <motion.h1 className="text-h1 font-kids font-bold text-brand-ink mb-8 tracking-tight">
            About <span className="text-brand-teal italic">Autism.</span>
          </motion.h1>
          
          <div className="space-y-6 text-body-lg text-brand-ink font-semibold mb-12 drop-shadow-sm">
            <p>
              Autism Spectrum Disorder (ASD) is a complex condition that includes a wide range of social and communication skills.
            </p>
            <p className="text-brand-teal border-l-4 border-brand-mint pl-6 italic font-bold text-h4">
              "Seeing the world beautifully differently."
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-6">
            <Link to="/contact" className="bg-brand-teal text-white px-10 py-5 rounded-full font-bold text-h4 hover:bg-brand-ink transition-all shadow-xl hover:-translate-y-1 flex items-center gap-3 group">
            ...
            <a href="tel:8554648559" className="flex items-center gap-3 text-brand-ink font-extrabold group">
              ...
              <span className="text-h4">855-464-8559</span>
            </a>
          </div>
        </motion.div>
...
<section className="py-32 px-6 lg:px-12 bg-white">
  <div className="max-w-7xl mx-auto">
    <div className="text-center mb-24">
      <h2 className="text-h1 font-kids font-bold text-brand-ink mb-8">
      ...
      {[
        ...
        >
          ...
          >
            ...
            ) : null} {/* Slightly larger icon */}
          </motion.div>
          
          <h3 className="relative z-10 text-h2 font-kids font-bold mb-6 text-brand-ink group-hover:text-white transition-colors duration-500">{item.title}</h3>
          <p className="relative z-10 text-body-lg text-brand-sage font-medium group-hover:text-white/80 transition-colors duration-500">
            {item.desc}
          </p>
        </motion.div>
      ))}
      ...
      {sections.map((section, index) => (
        ...
        >
          ...
          <div className="max-w-7xl mx-auto w-full grid lg:grid-cols-2 gap-16 items-center relative z-10">
            ...
            <h2 className="text-h2 font-kids font-bold mb-8 text-brand-ink">
                {section.title} <br />
                <span className={`${section.id === 'goals' ? 'text-brand-mint' : 'text-brand-teal'} italic`}>
                  {section.accent}
                </span>
              </h2>
              <p className={`text-body-lg font-medium opacity-90 max-w-xl ${section.id === 'goals' ? 'text-white' : 'text-brand-sage'}`}>
                {section.content}
              </p>
            </motion.div>
            ...
            >
              {section.highlight && (
                <motion.div 
                  ...
                  className="bg-white p-10 rounded-[60px] shadow-2xl border-l-[12px] border-brand-teal"
                >
                  <p className="text-h2 font-kids font-bold text-brand-ink italic">
                    "{section.highlight}"
                  </p>
                </motion.div>
              )}
                ...
                <div className="space-y-6">
                  {section.items.map((item, i) => (
                    <motion.div 
                      key={i}
                      whileHover={{ x: 20 }}
                      className="bg-white p-8 rounded-[40px] shadow-lg flex items-center gap-6 border border-gray-50 focus-within:ring-2 focus-within:ring-brand-teal"
                    >
                      <div className="w-16 h-16 rounded-2xl bg-brand-teal text-white flex items-center justify-center shadow-lg">
                        {item.icon}
                      </div>
                      <span className="text-h3 font-kids font-bold text-brand-ink">{item.title}</span>
                    </motion.div>
                  ))}
                </div>
              )}
                ...
                <div className="grid grid-cols-1 gap-4">
                  {section.goals.map((goal, i) => (
                    <motion.div 
                      ...
                      className="bg-white/10 backdrop-blur-md p-8 rounded-[40px] border border-white/20 flex items-center gap-6"
                    >
                      <CheckCircle2 size={32} className="text-brand-mint" />
                      <span className="text-h3 font-bold text-white">{goal}</span>
                    </motion.div>
                  ))}
                </div>
              )}
      ...
      <section className="relative py-40 px-6 bg-white overflow-hidden">
        <div className="max-w-5xl mx-auto text-center">
            <motion.div
             ...
             className="bg-[#FCFAF7] p-16 lg:p-24 rounded-[80px] border-2 border-dashed border-brand-mint relative shadow-inner"
           >
              <div className="inline-flex items-center gap-2 px-6 py-2 bg-brand-mint text-brand-teal rounded-full text-label mb-8">
                <Star size={16} fill="currentColor" /> Key Principle
              </div>
              <h3 className="text-h1 font-kids font-bold text-brand-ink mb-8">Positive Reinforcement.</h3>
              <p className="text-body-lg text-brand-sage font-medium mb-12">
                When a desired behavior is followed by a reward, that behavior is more likely to occur again. It's the engine of learning.
              </p>
              <div className="bg-white p-10 rounded-[40px] shadow-xl inline-block max-w-2xl border border-brand-teal/5">
                 <p className="text-brand-teal font-bold text-h4 italic">
...
<section className="relative py-32 bg-brand-ink text-white rounded-[100px] mx-6 overflow-hidden">
  ...
  <div className="max-w-7xl mx-auto px-12 grid lg:grid-cols-12 gap-16 items-center relative z-10">
    ...
    <div className="lg:col-span-6 space-y-12">
      <div className="space-y-6">
        <h2 className="text-h1 font-kids font-bold">
          Identifying <br />
          <span className="text-brand-mint italic">the Signs.</span>
        </h2>
        <p className="text-body-lg text-white/70 font-medium">
      ...
      <div className="grid sm:grid-cols-2 gap-6 relative">
        {[
          ...
          >
            ...
            <span className="font-bold text-h4 leading-tight">{item.text}</span>
          </motion.div>
        ))}
      </div>
    </div>
    ...
    <div className="lg:col-span-6 relative">
      ...
      <section className="py-32 bg-brand-mint/10 rounded-[100px] mx-6 shadow-sm border border-brand-teal/5">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
          <div>
            <motion.div initial="hidden" whileInView="visible" variants={fadeInUp} viewport={{ once: true }}>
              <div className="inline-flex items-center gap-2 px-5 py-2 bg-brand-teal text-white rounded-full text-label mb-8">
                <ShieldCheck size={16} /> Clinical Assessments
              </div>
              <h2 className="text-h1 font-kids font-bold text-brand-ink mb-10 leading-tight">
                The Path to <br /><span className="text-brand-teal italic">Diagnosis.</span>
              </h2>
              <div className="space-y-8 text-body-lg text-brand-sage font-medium">
                <p>
                ...
                <div className="p-8 bg-white rounded-[40px] border-2 border-dashed border-brand-teal/20 relative shadow-lg">
                  <Sparkles className="absolute -top-4 -right-4 text-brand-teal" size={32} />
                  <p className="text-brand-ink font-bold mb-4 italic text-h4">
                    "We're here to help you navigate this step."
                  </p>
                  <p className="text-body text-brand-sage">
              ...
              <div className="mt-12">
                <Link to="/contact" className="inline-flex items-center gap-3 bg-brand-ink text-white px-10 py-5 rounded-2xl font-kids font-bold text-h4 hover:bg-brand-teal transition-all shadow-xl group">
          ...
          <div className="relative">
            ...
            {/* Floating Badge */}
            <div className="absolute top-10 -left-10 bg-white p-8 rounded-[40px] shadow-2xl border border-brand-teal/5 max-w-[200px] rotate-[-5deg]">
               <p className="text-brand-teal font-kids font-bold text-h4 leading-tight">Expert Referral Network</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-40 max-w-5xl mx-auto px-6">
        <h2 className="text-h1 font-kids font-bold text-brand-ink mb-20 text-center">Your Questions, <span className="text-brand-teal">Answered.</span></h2>
        <div className="space-y-6">
          {faqData.map((faq, index) => (
            <div key={index} className="bg-white rounded-[45px] border border-brand-teal/5 overflow-hidden shadow-sm hover:shadow-md transition-all">
              <button 
                ...
                className="w-full flex items-center justify-between p-10 text-left font-bold text-h4 font-kids hover:text-brand-teal transition-colors group"
              >
                {faq.question}
              ...
              <AnimatePresence>
                {openFAQ === index && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
                    <div className="px-10 pb-10 text-body text-brand-sage font-medium border-t border-brand-teal/5 pt-8">
                      {faq.answer}
                    </div>
                  </motion.div>
                )}
      ...
      <section className="py-24 px-6 lg:px-12 max-w-7xl mx-auto text-center">
        <div className="bg-brand-teal rounded-[80px] p-20 lg:p-32 text-white relative overflow-hidden shadow-3xl">
          ...
          <h2 className="text-h1 font-kids font-bold mb-10 relative z-10">Start the Journey.</h2>
          <p className="text-body-lg opacity-90 mb-16 max-w-3xl mx-auto relative z-10 font-medium">
            Schedule a consultation today to learn how our individualized ABA programs can support your child's unique growth.
          </p>
          <Link to="/contact" className="inline-block bg-white text-brand-teal px-16 py-7 rounded-[24px] font-kids font-bold text-h3 hover:bg-brand-mint hover:text-brand-ink transition-all shadow-xl relative z-10">
            Book a Consultation
          </Link>
        </div>
      </section>

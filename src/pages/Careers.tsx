    ...
    <div className="pt-32 lg:pt-48 pb-16 lg:pb-32 bg-brand-cream overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-12 relative">
        ...
        <div className="max-w-4xl mb-16 lg:mb-32 relative z-10 text-center lg:text-left">
          ...
          >
            <span className="text-brand-teal text-label mb-6 block font-kids">Careers at Auvia</span>
            <h1 className="text-h1 font-kids font-bold text-brand-ink mb-8 lg:mb-10">
              A Supportive Team, <br className="hidden lg:block" />
              <span className="text-brand-teal italic">A Happy Family.</span>
            </h1>
            <p className="text-body-lg text-brand-sage font-medium max-w-2xl mx-auto lg:mx-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 mb-24 lg:mb-40 relative z-10">
          {benefits.map((benefit, i) => (
            ...
            >
              ...
              <h3 className="text-h3 font-kids font-bold text-brand-ink mb-6">{benefit.title}</h3>
              <p className="text-brand-sage font-medium text-body">{benefit.text}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="mb-40 relative z-10">
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            <div>
              <h2 className="text-h2 font-kids font-bold text-brand-ink mb-12">Our Culture is Rooted in <span className="text-brand-teal italic">Our Values.</span></h2>
              <div className="grid sm:grid-cols-2 gap-8">
                {values.map((value, i) => (
                  <div key={i} className="space-y-4">
                    ...
                    <h4 className="text-h4 font-kids font-bold text-brand-ink">{value.title}</h4>
                    <p className="text-brand-sage font-medium text-body-sm leading-relaxed">{value.text}</p>
                  </div>
                ))}
              </div>
            </div>
            <div className="relative">
              ...
              <div className="absolute -bottom-10 -right-10 bg-brand-peach p-8 rounded-[40px] shadow-xl max-w-xs border-4 border-white">
                ...
                <p className="text-brand-sage text-body-sm font-medium">We believe that taking care of our team is the first step in taking care of our families.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Testimonials */}
        <div className="mb-40 relative z-10">
          <h2 className="text-h2 font-kids font-bold text-brand-ink mb-16 text-center">Hear From <span className="text-brand-teal italic">Our Team.</span></h2>
            ...
            {testimonials.map((t, i) => (
              <div key={i} className="bg-white p-12 rounded-[48px] shadow-lg border border-brand-teal/5 relative">
                <div className="flex items-center gap-6 mb-8">
                  ...
                  <div>
                    <h4 className="text-h3 font-kids font-bold text-brand-ink">{t.name}</h4>
                    <p className="text-brand-teal text-label">{t.role}</p>
                  </div>
                </div>
                <p className="text-brand-sage text-body-lg font-medium italic">"{t.quote}"</p>
        ...
        <div className="mb-24 lg:mb-40 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 lg:mb-16 gap-8">
            <div className="max-w-2xl">
              <h2 className="text-h2 font-kids font-bold text-brand-ink mb-6">Current Opportunities</h2>
              <p className="text-brand-sage font-medium text-body">Find your place in our growing family. We're looking for passionate individuals to join us in Dallas, Fort Worth, Plano, and Frisco.</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <button onClick={() => setLocationFilter(locationFilter === 'all' ? 'clinical' : 'all')} className={`px-5 py-2 rounded-full font-bold text-label shadow-sm border transition-all ${locationFilter === 'all' ? 'bg-brand-teal text-white border-brand-teal' : 'bg-white text-brand-teal border-brand-teal/5 hover:border-brand-teal'}`}>All Locations</button>
              <button onClick={() => setRoleFilter(roleFilter === 'all' ? 'clinical' : 'all')} className={`px-5 py-2 rounded-full font-bold text-label shadow-sm border transition-all ${roleFilter === 'clinical' ? 'bg-brand-teal text-white border-brand-teal' : 'bg-white text-brand-teal border-brand-teal/5 hover:border-brand-teal'}`}>Clinical Roles</button>
            </div>
          </div>
              ...
              >
                <div className="mb-8 lg:mb-0 lg:max-w-2xl">
                  <div className="flex items-center gap-4 mb-4">
                    <h3 className="text-h3 font-kids font-bold text-brand-ink group-hover:text-brand-teal transition-colors duration-500">{job.title}</h3>
                    <span className={`px-4 py-1 rounded-full text-label bg-brand-mint/20 text-brand-teal`}>{job.type}</span>
                  </div>
                  <p className="text-brand-sage font-medium text-body mb-4">{job.description}</p>
                  <div className="flex gap-6 text-label font-kids text-brand-sage/60">
        ...
        <div className="bg-brand-teal rounded-[40px] lg:rounded-[60px] p-10 md:p-16 lg:p-32 text-center text-white relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            <h2 className="text-h1 font-kids font-bold mb-8 lg:mb-10">
              A Warm <br className="hidden lg:block" />
              <span className="text-brand-peach italic">Career Path.</span>
            </h2>
            <p className="text-white/80 text-body-lg mb-12 lg:mb-16 font-medium">
